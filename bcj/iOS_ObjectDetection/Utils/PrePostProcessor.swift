...
class PrePostProcessor : NSObject {
    ...
    // BCJ:  by examining output from detect.py predictio for df2.pt
    static let outputRow = 25200//15120 // as decided by the YOLOv5 model for input image of size 640*640
    static let outputColumn = 9 // left, top, right, bottom, score and 4 class probability
    ...
    static func outputsToNMSPredictions(outputs: [NSNumber], imgScaleX: Double, imgScaleY: Double, ivScaleX: Double, ivScaleY: Double, startX: Double, startY: Double) -> [Prediction] {
        ...
        for i in 0..<outputRow {
            if Float(truncating: outputs[i*outputColumn+4]) > threshold {
                ...
                let prediction = Prediction(classIndex: cls, score: Float(truncating: outputs[i*9+4]), rect: rect) // BCJ modified
